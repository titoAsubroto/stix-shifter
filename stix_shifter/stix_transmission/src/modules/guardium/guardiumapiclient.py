...
#
        ...
        if proxy is not None:
            ...
            if proxy.get('x_forward_proxy', None) is not None:
                ...
                if proxy.get('x_forward_proxy_auth', None) is not None:
                    self.headers['x-forward-auth'] = proxy.get('x_forward_proxy_auth')
    ...
    def get_credential(self):
        # Subroto -- Assumption: credential object is set prior calling this function
        # if self.credentail is None then we need to read it from a file temporarily
        # In the future we will throw error
        # if credential is None
        username = self.connection.get("username", "admin")
        host = self.connection.get("host")
        if self.credential is None:
            path = "./stix_shifter/stix_transmission/src/modules/guardium/credentials/"
            ...
            if exists:
                ...
                logging.info("Credential not provided or could not get from the - ", + credFile)
                raise IOError(3001, "Credential not provided or could not get from the - ", credFile)
        else:
    ...
    def fetch_accessToken(self):
        ...
        if (response.code != 200):
            ...
            self.authorization = json.loads('{"access_token":"' + jResp.get("access_token") + '", "expiresTimestamp":' + str(tExp) + '}')
        return successVal
    ...
    def get_accessToken(self):
        ...
        if (self.authorization is not None):
            ...
            if self.isTimestampValid((self.authorization).get("expiresTimestamp")):
                auth = "Bearer " + (self.authorization).get("access_token")
                successVal = True
            ...
            logging.info("Authorization token is not found. Get a new access token")
#
...
#
        if(self.query is None or self.authorization is None or self.credential is None):
            logging.Info("Could not generate search id because 'query' or 'authorization token' or 'credential info' is not available.")
#
        else:
            id_str = '{"query": ' + json.dumps(self.query) + ', "credential" : ' + json.dumps(self.credential) + \
                     ', "authorization" : ' + json.dumps(self.authorization) + '}'
            print(id_str)
...
#
    ...
    def decode_searchId(self):
        ...
        try:
            ...
            raise IOError(3001, "Could not decode search id content - ", + self.search_id)
#
    ...
    def create_search(self, query_expression):
        ...
        if (self.get_accessToken()):
            ...
            if (response != None):
                ...
                respObj.error_type = ""
                respObj.status_code = 200
                content = '{"search_id": "' + str(response) + '", "data": {"message":  "Search id generated."}}'
                #print(content)
                respObj._content = bytes(content, 'utf-8')
...
#
    ...
    def get_status(self, search_id):
        ...
        if (self.fetch_accessToken()):
            ...
            respObj.error_type = ""
            respObj.status_code = 200
            content = '{"search_id": "' + search_id + '", "progress":"Completed", "status":"COMPLETED", "data": {"message":"Completed for the search id provided."}}'
            respObj._content = bytes(content, 'utf-8')
        else:
            respObj.code = "503"
            respObj.error_type = "Service Unavailable"
...
#  replacement indexFrom and fetchSize
            ...
            logging.debug("Response code: " + str(response.response.status_code) + "\n")
