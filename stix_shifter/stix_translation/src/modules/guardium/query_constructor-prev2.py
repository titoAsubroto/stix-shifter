...
#        ComparisonComparators.Matches: 'LIKE',
    ...
    def __init__(self, pattern: Pattern, data_model_mapper):
        ...
        with open("./stix_shifter/stix_translation/src/modules/guardium/json/guardium_report_params_map.json", 'r') as f_repm:
            self.REPORT_PARAMS_MAPS = json.loads(f_repm.read())

    def substitute_ParamsPassed(self, reportDefs)
        ...
        for reportName in reportDefs:
            ...
            for param in report["reportParameter"]
                if param not in self.reportParamsPassed:
                    report["reportParameter"][param] = report["reportParameter"][param]["default"]
                else:
                    if "transformer" in report["reportParameter"][param]:
                        transformer = self.transformers[report["reportParameter"][param]["transformer"]]
                        report["reportParameter"][param] = transformer.transform(self.reportParamsPassed[param])
                    else:
                        report["reportParameter"][param] = self.reportParamsPassed[param]
...
#
    def get_report_params(self):
        ...
        if(dataCategory is not None):
            if dataCategory not in self.REPORT_PARAMS:
                ...
                reportDefs = self.REPORT_DEF[dataCategory]
            #replace
            #   params
            ...
            logging.info("'dataCategory' is not supplied. It is NONE")
            reports = None
...
def translate_pattern(pattern: Pattern, data_model_mapping):
    ...
    logging.debug("Translate Pattern: " + str(pattern))
...
#Subroto: I did not change the code much just adapted to get the report parameters
    ...
    if (reportHeader != None):
        ...
        logging.debug("Report params Header: " + str(reportHeader))
